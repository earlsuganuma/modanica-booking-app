...
async function payjpFetch(path, { method = "GET", params } = {}) {
  ...
  let res;
  try {
    res = await fetch(`https://api.pay.jp/v1${path}`, init);
  } catch (fetchErr) {
    // DEBUG: fetch自体が失敗した場合の詳細（原因調査のため一時的）
    const err = new Error("PAY.JPへの通信自体に失敗しました。");
    err.debugInfo = { stage: "fetch", path, name: fetchErr && fetchErr.name, message: fetchErr && fetchErr.message, cause: fetchErr && fetchErr.cause && String(fetchErr.cause) };
    throw err;
  }
  ...
  try {
    ...
    const err = new Error("PAY.JPレスポンスのJSON解析に失敗しました。");
    err.debugInfo = { stage: "parse", path, status: res.status, rawText: rawText.slice(0, 500) };
    throw err;
  }
  if (!res.ok) {
    const err = new Error((data && data.error && data.error.message) || "PAY.JPとの通信に失敗しました。");
    err.payjpError = data && data.error;
    // DEBUG: PAY.JPエラーレスポンスの詳細（原因調査のため一時的）
    err.debugInfo = { stage: "response", path, status: res.status, data };
    throw err;
  }
  return data;
}
