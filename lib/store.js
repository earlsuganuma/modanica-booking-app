...
async function load() {
  const [
    ...
    query("SELECT * FROM options ORDER BY sort_order"),
  ...
  return {
    ...
    reservations: reservationsRes.rows.map((r) => ({
      ...
      payjpChargeId: r.payjp_charge_id,
...
async function save(data) {
  ...
  await withTransaction(async (client) => {
    ...
    for (const p of d.plans) {
      await client.query(
        `INSERT INTO plans (id, code, category, name, exclusivity, time_type, min_guests, max_guests, base_price, description, booking_open_days_before, final_booking_deadline_days_before, slot_prices, sort_order, images)
    ...
    for (const o of d.options) {
      await client.query(
        `INSERT INTO options (id, name, price, unit, max_quantity, unit_label, description, image_url, sort_order) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
        [
          o.id,
          o.name,
          o.price ?? 0,
    ...
    for (const r of d.reservations) {
      await client.query(
        `INSERT INTO reservations (id, plan_id, resource_ids, option_ids, option_quantities, start_datetime, end_datetime, guest_count, customer_name, customer_email, customer_tel, note, status, total_price, created_at, payment_status, payjp_charge_id, payment_amount, refunded_amount, card_brand, card_last4)
